...
class hand_control():
    ...
    finger_move_offset = np.array([0., 0.01, 0.01, 0.01])
    ...
    obj_relative_quat = [0., 0., 0., 0.]
    obj_pos_pixels = [0, 0]
    start_obj_rpy = [0,0,0]
    start_obj_quat = [0, 0, 0, 0]
    R = []
    count = 1
    max_load = 180.0
    max_drop = 5
    ...
    grasped_object_id = 7
    object_map = {0: 'circle', 3: 'hexagon', 4: 'ellipse', 5: 'rectangle', 6: 'square', 9: 'box', 11: 'wire', 10: 'gum', 7: 'star'}
        ...
        if self.grasped_object_id not in detected_id:
            ...
            for detect in detection_array:
                ...
                if detect.id[0] == self.grasped_object_id:
                    ...
                    self.obj_relative_quat = [pose.orientation.x,pose.orientation.y,pose.orientation.z,pose.orientation.w]
    ...
    def CloseGripperOld(self, msg):
        # GRASP OBJECT
        # CHECK OVERHEAT
        count = 0
        to_add = np.array([a for a in self.finger_move_offset])
        ...
        for i in range(1000):
            # CHECK LOAD
            if np.all(np.abs(self.gripper_load[1:]) > closed_load) :
                # rospy.loginfo('[hand_control] Object grasped.')
                self.gripper_status = 'closed'
                break
            if np.any(np.abs(self.gripper_load[1:]) > closed_load):
                count += 1
            if count > 5:
                to_add = np.where(np.abs(self.gripper_load) > closed_load, self.finger_move_offset, self.finger_move_offset*(3/2))
                rospy.logdebug(self.gripper_load[1:])
                count = 0
            # CHECK GRASP ANGLE LIMITS
            desired = self.gripper_pos + to_add  # slow close [a for a in self.finger_move_offset]
    ...
    def CloseGripper(self, msg):
        ...
        for i in range(400):
            ...
            if count > 5:
                ...
                break
            # CHECK GRASP ANGLE LIMITS
            desired = self.gripper_pos + np.array([a for a in self.finger_move_offset])  # slow close [a for a in self.finger_move_offset]
            if np.any(desired > 0.7):
                rospy.logdebug('[hand_control] Desired angles out of bounds to grasp object.')
                break
            self.moveGripper(desired)
    ...
    def GetObservation(self, msg):
        # TODO: state -> obj_pos, obj_rpy, act_angles, act_torques
        #                   3       3           3           3

        obs = np.concatenate((self.obj_relative_pos,
                              self.obj_relative_rpy,
                              self.gripper_pos[1:],
                              self.gripper_load[1:],
                              self.target_pos[1:]))
        return {'state': obs}
